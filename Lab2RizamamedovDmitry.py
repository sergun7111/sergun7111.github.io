...
class Cars:
    ...
    def __init__(self, name, fares="Выключены", engine=None, color=None, doors="Закрыты", year=None, developer=None,
    ...
    def car_info(self):
        print(
            f'Модель: {self.name}. Цвет: {self.color}. Год производства: {self.year}. Двигатель: {self.engine}. Фары: {self.fares}. Двери: {self.doors}. Производитель: {self.developer}. Мощность: {self.power}. Стоимость: {self.price}')
...
class Interface:
    ...
    def GUI(self):
        while True:
            ...
            def print_car_count():
                print("Количество авто:", len(mass), "\n")  # Добавляет количество машин в список авто
            ...
            if choice == "1":
                fileSize5 = os.path.getsize(
                    "d:/Мои проекты/Лабораторные работы/Основы программной инженерии/Лаб2_Ризамамедов Д.А. (Авто-1)Папка/file.pkl")
                ...
                with open("file.pkl", "wb") as fp:
                    pickle.dump(mass, fp)
                fileSize6 = os.path.getsize(
                    "d:/Мои проекты/Лабораторные работы/Основы программной инженерии/Лаб2_Ризамамедов Д.А. (Авто-1)Папка/file.pkl")
                if fileSize6 > fileSize5:
                    print("Добавление прошло успешно!")
            elif choice == '2':
                vrizm1 = os.stat(
                    "d:/Мои проекты/Лабораторные работы/Основы программной инженерии/Лаб2_Ризамамедов Д.А. (Авто-1)Папка/file.pkl").st_mtime
                fileSize3 = os.path.getsize(
                    "d:/Мои проекты/Лабораторные работы/Основы программной инженерии/Лаб2_Ризамамедов Д.А. (Авто-1)Папка/file.pkl")
                for i in range(len(mass)):
                    print(i + 1, ') ', mass[i].name, ' (', "Цвет: ", mass[i].color, ', ', "Год: ", mass[i].year, ', ',
                          "Двигатель: ", mass[i].engine, ', ', "Производитель: ", mass[i].developer, ', ', "Мощность: ",
                          mass[i].power, ', ', "Стоимость: ", mass[i].price, ', ', "Двери: ", mass[i].doors, ', ',
                          "Фары: ", mass[i].fares, ')', sep='')
                ...
                if len(mass) == 0:
                    ...
                    for i in range(len(mass)):
                        print(i + 1, ') ', mass[i].name, ' (', "Цвет: ", mass[i].color, ', ', "Год: ", mass[i].year,
                              ', ', "Двигатель: ", mass[i].engine, ', ', "Производитель: ", mass[i].developer, ', ',
                              "Мощность: ", mass[i].power, ', ', "Стоимость: ", mass[i].price, ', ', "Двери: ",
                              mass[i].doors, ', ', "Фары: ", mass[i].fares, ')', sep='')
                    choice3 = int(input())
